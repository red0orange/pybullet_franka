...
def get_collision_file(visual_file, resolution=10000):
    visual_file = path.Path(visual_file)
    collision_file = visual_file.stripext() + ".convex" + visual_file.ext
    if not collision_file.exists():
        # vhacd_path = os.path.dirname(str(visual_file))  @note TODO
        # cmd = f"{vhacd_path}/TestVHACD {visual_file} -o {collision_file} -r {resolution}"
        vhacd_path = "/home/huangdehao/github_projects/multi_view_rearr/pybullet_pipeline/vhacd/build"
        cmd = f"{vhacd_path}/TestVHACD {visual_file} -r {resolution}"
        subprocess.check_output(shlex.split(cmd))
        shutil.move("decomp.obj", collision_file)
