...
def get_grasp_poses(env):
    ...
    if _utils.get_class_id(env.fg_object_id) == 2:
        ...
        edge_mask = cv2.dilate(np.uint8(edge_mask) * 255,
...
def plan_reorient(env, grasp_pose, reorient_pose):
    ...
    for dg in np.random.uniform(-np.pi, np.pi, size=(8,)):
        ...
        c.rotate([0, 0, dg])
        j = env.pi.solve_ik(c.pose, n_init=5, validate=True,
    ...
    ee_af_to_world = env.pi.get_pose(end_effortor_name)
    ...
    c = reorientbot.geometry.Coordinate(*env.pi.get_pose(end_effortor_name))
    ...
    if not env.pi.validatej(js[0], obstacles=bg_object_ids + [env.fg_object_id]):
...
def execute_reorient(env, result):
    ...
    for _ in env.pi.grasp(min_dz=0.08, max_dz=0.12, rotation_axis=True, speed=0.001):
        pp.step_simulation()
        if pp.has_gui():
            time.sleep(pp.get_time_step())
...
def get_static_reorient_poses(env):
    ...
    index = np.argmin(np.linalg.norm(
        pcd_in_obj - pcd_in_obj.mean(axis=0), axis=1))
