...
def my_visualize_grasps(full_pc, grasp_Ts, scores, pc_colors=None, gripper_width=0.08):
    ...
    colors = [cm(0.5*score)[:3] for score in scores]
    gripper_openings_k = np.ones(len(grasp_Ts))*gripper_width
    draw_grasps(grasp_Ts, np.eye(4), colors=colors, gripper_openings=gripper_openings_k)    
